...
class News_api extends Base_api
{
     ...
     * @var string
     */
    public $validator_name = 'News_api_validator';

    /**
     * News get list API Spec N-010
     *
     * @param array $params
    ...
    /**
     * Get news detail API Spec N-020 & API Spec UNW-011
     *
     * @param array $params
     * @internal param $id of news
     *
     * @return array
     */
    public function get_detail($params = [])
    {
        ...
        // Only administrator can view before_published and expired news
        if(!$this->operator()->is_administrator()) {
            $this->news_model->with_available();
        } else {
            // Guest can only view public news
            if(!$this->operator()->is_login()) {
                $this->news_model->with_public();
            }
        }
        ...
        // Get news detail
        $res = $this->news_model
            ->select('id, title, content, status, started_at, ended_at, created_at')
            ->find($params['id']);
        ...
        return $this->true_json($this->build_responses($res, ['detail']));
    }

    /**
     * Add news API Spec N-110
     *
     * @param array $params
     * @internal param string $title of news
     * @internal param string $content of news
     * @internal param datetime $started_at of news
     * @internal param datetime $ended_at of news
     * @internal param string $status of news (private|public)
     * @return array
     */
    public function create($params = [])
    {
        ...
        $v = $this->validator($params);
        $v->require_login();
        $v->require_permissions('NEWS_CREATE');
        $v->set_rules('title', '件名', 'required|max_length[255]');
        $v->set_rules('content', '本文', 'required');
        $v->set_rules('started_at', '公開期間（開始）', 'required|datetime_format');
        ...
        // Use library html purifier to clean bad html in content
        $this->load->library('Html/Html_purifier');
        $params['content'] = $this->html_purifier->clean_html($params['content']);

        // Load model
        $this->load->model('news_model');

        /** @var object $res Create user */
        $res = $this->news_model->create($params, [
            'return' => TRUE
        ]);
    ...
    /**
     * Edit news API Spec N-111
     *
     * @param array $params
     * @internal param $id of news
     * @internal param $title of news
     * @internal param $content of news
     * @internal param $started_at of news
     * @internal param $ended_at of news
     * @internal param $status of news (public|private)
     *
     * @return array
     */
    public function edit($params = [])
    {
        ...
        $v = $this->validator($params);
        $v->require_login();
        $v->require_permissions('NEWS_UPDATE');
        $v->set_rules('id', 'ニュースID', 'required|integer');
        $v->set_rules('title', 'タイトル', 'required|max_length[255]');
        $v->set_rules('content', '本文', 'required');
        ...
        $res = $this->news_model->update($params['id'], $params, [
            'return' => TRUE
        ]);
    ...
    public function delete($params = [])
    {
        ...
        $v = $this->validator($params);
        $v->require_login();
        $v->require_permissions('NEWS_DELETE');
        ...
        $news = $this->news_model->where('id', $params['id'])->first();
        ...
        // Remove news
        $this->news_model->destroy($news->id);
...
class News_api_validator extends Base_api_validation {
